import { IconClock, IconEdit, IconEye, IconMail, IconPencil, IconPlug, IconPlus, IconPrinter, IconSearch, IconTrash, IconX } from '@tabler/icons-react';
import { IconEditOff } from '@tabler/icons-react';
import { useContext, useEffect } from 'react';
import OrdersContext from '../context/orders/OrdersContext';
import { getOrders } from '../context/orders/OrdersAction';

function OrdersPagePrint() {
  ...
  const getOrderState = (param) => {
    switch(param) {
      case 'approved':
       return (
        <button className="flex items-center w-fit whitespace-nowrap rounded-full px-3 py-1.5 bg-blue-600 text-white text-sm whitespace-nowrap">
          <IconPrinter className="mr-1" size={14} />
          Распечатать
        </button>
       );
      case 'denied':
      ...
      case 'pending':
      return (
        <div className="flex items-center w-fit whitespace-nowrap rounded-full px-3 py-1.5 bg-slate-200 text-slate-800 text-sm whitespace-nowrap">
...
export default OrdersPagePrint